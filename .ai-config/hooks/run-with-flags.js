...
function getPluginRoot() {
  ...
  // Installed: project/.ai-config/hooks/run-with-flags.js → project root is two levels up
  if (path.basename(path.dirname(here)) === '.ai-config') {
    return path.resolve(here, '..', '..');
  }
  // Repo dev: repo/hooks/run-with-flags.js → repo root is one level up
  return path.resolve(here, '..');
}
