...
# 安装通用规则（所有项目必需）
cp -r rules/common ~/.cursor/rules/common

# 安装中文翻译版本（可选）
cp -r rules/zh ~/.cursor/rules/zh

# 根据项目技术栈安装语言特定规则
cp -r rules/typescript ~/.cursor/rules/typescript
cp -r rules/python ~/.cursor/rules/python
cp -r rules/golang ~/.cursor/rules/golang
cp -r rules/swift ~/.cursor/rules/swift
cp -r rules/php ~/.cursor/rules/php
```
