...
class LimitSchema(Schema):
    limits = fields.Dict(keys=fields.Str(), values=fields.Str(), load_default=dict)
class SpecSchema(Schema):
    ...
    pipeline     = fields.Str(required=False, allow_none=True)
    repository   = fields.Str(required=False, allow_none=True)
    resources    = fields.Nested(LimitSchema, required=False)
    
class StatusSchema(Schema):
    phase = fields.Str(required=False)


class SpaceSchema(Schema):
    apiVersion = fields.Str(required=True, validate=validate.Equal("devopstales.github.io/v1"))
    kind = fields.Str(required=True, validate=validate.Equal("Space"))
