...
from lib_functions.sso import SSOSererGet, SSOServerUpdate, SSOServerCreate, \
    get_auth_server_info
from lib_functions.k8s import k8sServerConfigGet, k8sServerConfigCreate, \
    k8sServerConfigUpdate, k8sServerDelete, k8sServerConfigList
...
@sso.route('/sso-config', methods=['GET', 'POST'])
@login_required
def sso_config():
...
@sso.route('/cluster-config', methods=['GET', 'POST'])
@login_required
def k8s_config():
    if request.method == 'POST':
        request_type = request.form['request_type']
